...
void build(int i, int l, int r){
    ...
    tree[i]=max(tree[i+i+1],tree[i+i+2]);
}
...
int query(int i, int l, int r, int ql, int qr){
    ...
    if(qr<l||r<ql||l>r)
        return 0;
    int m=(l+r)>>1;
    int f=max(query(2*i+1, l, m, ql, qr) , query(2*i+2, m+1, r, ql, qr));
    return f;
}
...
int query_hld2(int u, int v){
    ...
    while(chainInd[u] != chainInd[v]){
        ans=max(ans,query(0, 0, ptr-1, posInBase[u], posInBase[chainHead[chainInd[u]]]));
        u=dp[chainHead[chainInd[u]]][0];
    }
    if(u!=v)
        ans=max(ans,query(0, 0, ptr-1, posInBase[u], posInBase[v]+1));
    return ans;
}
...
sp;
    ...
    while(t--){
        ...
        while(1){
            ...
            cin>>x>>y;
            if(c[0]=='C'){
                int z=edges[x-1].second.first;
                update(0,0,ptr-1,posInBase[z],y);
            }
            else
