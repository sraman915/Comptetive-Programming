...
vector<PT> CircleLineIntersection(PT a, PT b, PT c, double r) {
  vector<PT> ret;
  b = b-a;	  a = a-c;
  double A = dot(b, b);	double B = dot(a, b);	double C = dot(a, a) - r*r;	  double D = B*B - A*C;
  if (D < -EPS) return ret;
...
// This code computes the area or centroid of a (possibly nonconvex) polygon,
//assuming that the coordinates are listed
...
int main(){
sp;
PT a(0,0),b(4,0),c(4,4),e(2,8),d(0,4);
vector<PT> p;
p.pb(a);
p.pb(b);
p.pb(c);
